# ADR-001：保持 observability-otel 为独立 Gnalloy 模块
...
`observability-otel` 作为 `gnalloy.org/observability-otel` 发布，并拥有该职责：Gnalloy observability metrics 的 OpenTelemetry 适配器。
